...
// mpicc -g -Wall -lm -o task_32 PartA/task_32.c
// mpirun -np 4 gdb ./task_32 testcases/input_testcase5.txt testcases/task_32_out
// mpirun -n 1 -slots=64 ./task_32 testcases/input_testcase testcases/task_32_out

// mpirun -n 16 ./task_32 testcases/input_testcase_large5 testcases/task_32_out
// mpirun -n 16 ./task_32 testcases/input_testcase800 testcases/task_32_out

int main(int argc, char *argv[]) {

    FILE *fp;
    int *a, *b, *c, *local_a, *local_b, *local_c, *buffer_a, *buffer_b;
    ...
    MPI_Request re1, re2, re3, re4;


    local_a = (int *) malloc(local_n * local_n * sizeof(int));
    local_b = (int *) malloc(local_n * local_n * sizeof(int));
    buffer_a = (int *) malloc(local_n * local_n * sizeof(int));
    buffer_b = (int *) malloc(local_n * local_n * sizeof(int));
    ...
    for (int i = 0; i < dim[0]; i++) {
        // non-blocking
        if (i % 2 == 0) {
            MPI_Isend(&local_a[0], local_n * local_n, MPI_INT, left, 1, comm, &re1);
            MPI_Irecv(&buffer_a[0], local_n * local_n, MPI_INT, right, 1, comm, &re2);
            MPI_Isend(&local_b[0], local_n * local_n, MPI_INT, up, 1, comm, &re3);
            MPI_Irecv(&buffer_b[0], local_n * local_n, MPI_INT, down, 1, comm, &re4);
            ...
            MPI_Isend(&buffer_a[0], local_n * local_n, MPI_INT, left, 1, comm, &re1);
            MPI_Irecv(&local_a[0], local_n * local_n, MPI_INT, right, 1, comm, &re2);
            MPI_Isend(&buffer_b[0], local_n * local_n, MPI_INT, up, 1, comm, &re3);
            MPI_Irecv(&local_b[0], local_n * local_n, MPI_INT, down, 1, comm, &re4);

            for (int x = 0; x < local_n; x++) {
                for (int y = 0; y < local_n; y++) {
                    for (int z = 0; z < local_n; z++) {
                        local_c[x * local_n + y] += buffer_a[x * local_n + z] * buffer_b[z * local_n + y];
                    }
                }
            }
